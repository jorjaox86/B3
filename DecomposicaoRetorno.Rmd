...
##Gerando Dados Complementares para o cálculo dos betas
Gerando $UR^M$, $NCF^M$ e $NDR^M$ 

```{r}
   tb6 <- decomposicaoRetornoOutroArquivo(dtICC_D,dadosR)
   tb6[,ICC := paste0(gsub('ICC_','',ICC),"(%)")]
   tb6[,Valor := round(Valor*100,2)]
   tb6 <- dcast(tb6,VAR+VARF~ICC,value.var = "Valor")
   tb6 <- as.data.frame(tb6)
   rownames(tb6) <- tb6$VARF
   
   kable(tb6[c(4,6,5,1,7,2,3),-c(1,2)],align=c(rep('c', 4)))
...
```{r}
   ...
   tb4 <- dcast(tb2,VARF~CASE,value.var = "Valor")
   tb4 <- as.data.frame(tb4)
   rownames(tb4) <- tb2$VARF
   kable(tb4[c(5,6,4,2,7,3,1),6:2],align=c(rep('c', 5)))
```
...
```{r}
   ...
   tb5 <- dcast(tb2,VARF~CASE,value.var = "Valor")
   tb5 <- as.data.frame(tb4)
   rownames(tb4) <- tb2$VARF
   kable(tb4[c(5,6,4,2,7,3,1),6:2],align=c(rep('c', 5)))
```
