...
```{r}
   tb2 <- betasTab2(dadosn, dtICC_D)
   colnames(tb2) <- c("GLS","CT","OJ","E","AVERAGE")
...
```
##TABELA 3
Análise dos betas a partir da variação da pontuação ESG em diferentes períodos
```{r}
   dtICCESG <- Tab3DeltaIndex(dtICC_D,esgTEMP)
   tb3 <- betasTab3(dadosn, dtICCESG)
   kable(tb3)
...
```{r}
   ...
   tb5 <- rbind(P10,P20,P30,P40,P50)
   tb5[,Valor := round(Valor*100,2)]
   tb5 <- dcast(tb5,VARF~CASE,value.var = "Valor")
   tb5 <- as.data.frame(tb5)
   rownames(tb5) <- tb5$VARF
   kable(tb5[c(5,6,4,2,7,3,1),6:2],align=c(rep('c', 5)))

   tb4_1 <- tab4Build(dadosn, dtICC_D,ano_0+1)
   tb4_2 <- tab4Build(dadosn, dtICC_D,ano_0)

   # tb4 <- BetaTab4Calc(tb4_1,tb4_2)
   
   # tb4_1 <- rbind(P10,P20,P30,P40,P50)
   ...
   kable(tb4_1[c(5,6,4,2,7,3,1),6:2],align=c(rep('c', 5)))
   
   # tb4_2 <- rbind(P10,P20,P30,P40,P50)
   tb4_2[,Valor := round(Valor*100,2)]
   tb4_2 <- dcast(tb4_2,VARF~CASE,value.var = "Valor")
   tb4_2 <- as.data.frame(tb4_2)
   rownames(tb4_2) <- tb4_2$VARF
   kable(tb4_2[c(5,6,4,2,7,3,1),6:2],align=c(rep('c', 5)))
   
   rowSums(tb4_2[, c("40%", "30%","20%")])
   
   
   info <- BetaTab4Calc(tb4_1, tb4_2, BetaNames)
   tb5_1 <- rbind(tb5,info)
...
```{r}
   tb5 <- tab5Build(dadosn, dtICC_D,BetaNames)
   rownames(tb5) <- tb5$VARF
   kable(tb5)
```
...
```{r}
   # tb6 <- decomposicaoRetornoOutroArquivo(dtICC_D,dadosR,interv = list(H=c(.5,1),L=c(0,.5)))[ICC == "ICC_GLS"]
   # 
   # # P50[,CASE := "50% (base case)"]
   # # 
   # # tb6 <- P50
   # tb6[,Valor := round(Valor*100,2)]
   # tb6 <- dcast(tb6,VAR+VARF~ICC,value.var = "Valor")
   # tb6 <- as.data.frame(tb6)
   # rownames(tb6) <- tb6$VARF
   # kable(tb6,align=c(rep('c', 5)))
   tb6 <- tab6Build(dadosn, dtICC_D)
   kable(tb6)
   class(tb6)
```
...
```{r}
   tb2
   tb7 <- tab7Build(dtGLS_ICC_0,dtCT_ICC_0,dtOJ_ICC_0,dtE_ICC_0,dsBR,esgB3,dadosn)
   kable(tb7)
   # tb7[,Environmental_Score := as.numeric(Environmental_Score)]
